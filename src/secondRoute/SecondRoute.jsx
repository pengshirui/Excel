...
const component = (props) => {
  const {args, data, binaryData, updateArgs, updateData, submit, submitUseManullayInputPattern, setData, csv, patterns, results, resultsRawData, zerosRawData, onesRawData, zero, one, two} = props;
  const regex = /^\d+(,\d+)*$/;
      ...
      <Row>
        <Col xs={6}>
          <form>
            ...
            <PanelGroup>
              <BallData b={binaryData} one={one} two={two} zero={zero} header="二进制数据 （二路为1，其他为0）" eventKey={0} bsStyle="success" />
            </PanelGroup>
