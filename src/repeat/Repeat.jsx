...
import { convertToRepeat } from '../repeat/Convert.js';
...
const enhance = compose(
  ...
  withHandlers({
    ...
    submit: ({ballNumber, csv, data, setBinaryData, setPatterns, setResultsRawData, setResults, setZerosRawData, setOnesRawData, setTwosRawData, setZero, setOne, setTwo   }) => () => {
      ...
      const bData = convertToRepeat(ballNumber, csv);
...
const component = (props) => {
  const {args, data, binaryData, updateArgs, updateDataByBtn, submit, submitUseManullayInputPattern,  patterns, results, resultsRawData, zerosRawData, onesRawData, twosRawData,  zero, one, two } = props;
  const regex = /^\d+(,\d+)*$/;
      ...
      <Row>
        <Col xs={6}>
          <form>
            ...
            <PanelGroup>
              <BallData b={binaryData} one={one} two={two} zero={zero} header="二进制数据 (中为0，隔为1，复为2）" eventKey={0} bsStyle="success" />
            </PanelGroup>
        ...
        <Col xs={6}>
          <PanelGroup>
            {resultsRawData && resultsRawData.length > 0 && resultsRawData.map((r, k) => (
              <ResultData pattern={patterns[k]} resultData={results[k]} resultRawData={r} header={"第"+(k+1)+"次复隔中结果"} eventKey={0} bsStyle="primary" key={k} 
...
export const Repeat = enhance(component);