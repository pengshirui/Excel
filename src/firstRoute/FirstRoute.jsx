      ...
      <Row>
        <Col xs={6}>
          <form>
            ...
            <PanelGroup>
              <BallData b={binaryData} one={one} two={two} zero={zero} header="二进制数据 （一路为1，其他为0）" eventKey={0} bsStyle="success" />
            </PanelGroup>
        ...
        <Col xs={6}>
          <PanelGroup>
            {resultsRawData && resultsRawData.length > 0 && resultsRawData.map((r, k) => (
              <ResultData pattern={patterns[k]} resultData={results[k]} resultRawData={r} header={"第"+(k+1)+"次1路结果"} eventKey={0} bsStyle="primary" key={k} 
